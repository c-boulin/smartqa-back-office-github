...
import { navigateToFilteredTests, type TestsDeepLinkFilters } from './navigateToFilteredTests';
...
interface StatCardConfig {
  ...
  accentClass: string;
  extraFilters: Pick<TestsDeepLinkFilters, 'status' | 'hasIssues'>;
}
...
const OverviewSummaryCards: React.FC<OverviewSummaryCardsProps> = ({
  ...
  const cards: StatCardConfig[] = [
    ...
    },
    {
      ...
      extraFilters: { hasIssues: true },
    },
  ];

  const openTests = (extra: Pick<TestsDeepLinkFilters, 'status' | 'hasIssues'>): void => {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {cards.map((card) => {
        const handleClick = (): void => openTests(card.extraFilters);
        ...
        return (
          <div
            ...
            aria-label={`View tests for ${card.label}`}
