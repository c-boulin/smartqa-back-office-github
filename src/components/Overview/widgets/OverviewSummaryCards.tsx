...
import { navigateToFilteredLaunches, type LaunchesDeepLinkFilters } from './navigateToFilteredLaunches';
...
interface StatCardConfig {
  ...
  accentClass: string;
  extraFilters: Pick<LaunchesDeepLinkFilters, 'status' | 'hasIssues'>;
}
...
const OverviewSummaryCards: React.FC<OverviewSummaryCardsProps> = ({
  ...
  const openLaunches = (extra: Pick<LaunchesDeepLinkFilters, 'status' | 'hasIssues'>): void => {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {cards.map((card) => {
        const handleClick = (): void => openLaunches(card.extraFilters);
