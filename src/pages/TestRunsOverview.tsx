  ...
  const handleExecutionResultChange = async (testCaseId: string, testRunId: string, newResultId: TestResultId, comment?: string, configurationId?: string) => {
    ...
    try {
      ...
      if (testRun) {
        ...
        const allTestCasesPassed = updatedTestRunTestCases.every(tc => tc.executionStatus === 1);
        const anyTestCaseFailed = updatedTestRunTestCases.some(tc => tc.executionStatus === 2);

        if (anyTestCaseFailed && testRun.state !== 4 && testRun.state !== 6) {
          // Any test case failed, move test run to "Rejected" (state 4)
          try {
            await testRunsApiService.updateTestRunState(testRunId, 4, testRun.testPlanId);
            setTestRuns(prevRuns => prevRuns.map(tr => tr.id === testRunId ? { ...tr, state: 4 } : tr));
